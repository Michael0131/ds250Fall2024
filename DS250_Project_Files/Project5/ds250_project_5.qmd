...
# Load the dataset
file_path = "StarWars_Cleaned_ML.csv"  # Correct file path
data = pd.read_csv(file_path)
...
# Filter respondents who have seen any film
seen_any_data = data[data['seen_any'] == "Yes"]

# Define movie columns based on dataset
movie_columns = [
...
seen_percentages = [
    seen_any_data[column].notna().sum() / len(seen_any_data) * 100
    for column in movie_columns
]

# Plot Visual 1: Movies Seen
plt.figure(figsize=(10, 6))
plt.barh(movie_columns, seen_percentages, color='blue')
plt.xlabel('Percentage')
plt.title("Which 'Star Wars' Movies Have You Seen?")
plt.gca().invert_yaxis()
plt.show()

# Best Star Wars Movie Analysis
# Assume respondents who ranked movies are stored under rank columns
rank_columns = [
    "rank_episode_1", "rank_episode_2", "rank_episode_3",
    "rank_episode_4", "rank_episode_5", "rank_episode_6"
]

# Calculate the percentage of respondents who ranked each movie as the best
best_movie_percentages = [
    (seen_any_data[column] == 1).sum() / len(seen_any_data) * 100
    for column in rank_columns
]

# Map movie titles to ranks for the chart
movie_titles = [
    "The Phantom Menace",
...
plt.figure(figsize=(10, 6))
plt.barh(movie_titles, best_movie_percentages, color='green')
plt.xlabel('Percentage')

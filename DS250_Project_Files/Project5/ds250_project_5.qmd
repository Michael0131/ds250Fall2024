...
print(f"Number of respondents who have seen all movies: {len(seen_all_data)}")

# -----------------------------------------------------------------------------
# --- Recreate Chart 1: "Percentage of Respondents Who Have Seen Each Movie" ---
# -----------------------------------------------------------------------------

# Extract the 'movies_seen' column and filter those who have seen at least one movie
seen_any_data = data[data['seen_any'] == "Yes"]

# Calculate the percentages of respondents who have seen each movie
seen_percentages = [
    round(seen_any_data['movies_seen'].str.contains(movie).sum() / len(seen_any_data) * 100)
    for movie in movie_titles
]

# Create a horizontal bar chart
plt.figure(figsize=(10, 6))
bars = plt.barh(short_titles, seen_percentages, color="skyblue")
plt.title("Percentage of Respondents Who Have Seen Each Movie")
plt.xlabel("Percentage")
plt.ylabel("Movies")
plt.xlim(0, 100)  # Set the x-axis limit for better alignment
...
# -----------------------------------------------------------------------------
# --- Recreate Chart 2: "What's the Best 'Star Wars' Movie?" ---
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# --- (Optional) Recreate Chart 3: "Who Shot First?" ---
# -----------------------------------------------------------------------------
...
# List of movies (used for filtering and labels)
movie_titles = [
    "Star Wars: Episode I  The Phantom Menace",
    "Star Wars: Episode II  Attack of the Clones",
    "Star Wars: Episode III  Revenge of the Sith",
...
# Function to check if all movies are seen (robust to NaNs)
def has_seen_all_movies(movies_string):
    if isinstance(movies_string, str):
        return all(movie in movies_string for movie in movie_titles)
    return False

# Filter for respondents who have seen all movies
seen_all_data = data[data['movies_seen'].apply(has_seen_all_movies)]
print(f"Number of respondents who have seen all movies: {len(seen_all_data)}")
...
# Plot the "Movies Seen" bar chart
plt.figure(figsize=(10, 6))
bars = plt.barh(short_titles, seen_percentages, color="skyblue")
plt.title("Percentage of Respondents Who Have Seen Each Movie")
plt.xlabel("Percentage")
...
# Identify the columns that hold movie rankings
...
# Filter the ranking columns from respondents who have seen all movies
rankings_seen_all = seen_all_data[ranking_column_names]


# Count the number of times each movie was ranked as the best (rank 1)
best_movie_counts = [0] * 6  # Initialize a list to store counts for each movie

for _, row in rankings_seen_all.iterrows(): # Iterate through rows
    for i, rank in enumerate(row): # Iterate through the values in each row
        if rank == 1: # check if the rank is 1 and if so then increase the count in the list
            best_movie_counts[i] += 1 # increase the best movie count for the movie in the specific index
...
print("Debug Information:")
for i, count in enumerate(best_movie_counts):
    print(f"Movie: {short_titles[i]}, Best Votes: {count}")
print(f"Total Best Votes: {total_best_votes}")
...
for bar, percentage in zip(bars, best_movie_percentages):
    plt.text(bar.get_width() + 1, bar.get_y() + bar.get_height() / 2, f"{percentage}%", va='center')
plt.title("What's the Best 'Star Wars' Movie?\nAccording to respondents who have seen all the movies")
plt.xlabel("Percentage")
plt.ylabel("Movies")
plt.xlim(0, 100)  # Adjust for better alignment

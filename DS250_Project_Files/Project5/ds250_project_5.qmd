...
# Split 'movies_seen' column into individual movie counts
movie_columns = [
...
# Initialize a dictionary to count occurrences of each movie
movie_counts = {movie: 0 for movie in movie_columns}

# Count occurrences of each movie
for movies in data["movies_seen"].dropna():
...
movie_percentages = {
    movie: (count / total_respondents) * 100 for movie, count in movie_counts.items()
}

# Plot the visual
plt.figure(figsize=(10, 6))
plt.barh(list(movie_percentages.keys()), list(movie_percentages.values()), color="blue")
...
# Aggregate rankings for "best" (ranked as 1)
rank_columns = [
    "Please rank the Star Wars films in order of preference with 1 being your favorite film in the franchise and 6 being your least favorite film.",
    "Unnamed: 10", "Unnamed: 11", "Unnamed: 12", "Unnamed: 13", "Unnamed: 14"
]

best_movie_counts = {movie: 0 for movie in movie_columns}

# Count the number of "1" rankings for each movie
for index, row in data.iterrows():
    for i, movie in enumerate(movie_columns):
        if i < len(rank_columns) and row[rank_columns[i]] == 1:
            best_movie_counts[movie] += 1

# Calculate percentages for "best" rankings
best_movie_percentages = {
    movie: (count / total_respondents) * 100 for movie, count in best_movie_counts.items()
}

# Plot the visual
plt.figure(figsize=(10, 6))
plt.barh(list(best_movie_percentages.keys()), list(best_movie_percentages.values()), color="green")
plt.xlabel("Percentage")
